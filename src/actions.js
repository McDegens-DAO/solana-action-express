...
// *********************************************************************************
app.get("/actions.json",(req,res)=>{let actions=JSON.parse(fs.readFileSync('actions.json','utf8'));res.send(JSON.stringify(actions));});app.get("/",(req,res)=>{res.setHeader('Content-Type','application/json');res.send(JSON.stringify('solana-action-express is running on '+proto+http_port));});let server=null;if(proto=="https"){const credentials={key:fs.readFileSync(ssl_key,'utf8'),cert:fs.readFileSync(ssl_crt,'utf8')};server=https.createServer(credentials,app);}else{server = http.createServer(app);} server.listen(port,()=>{console.log('solana-action-express is running on '+proto+http_port);});
