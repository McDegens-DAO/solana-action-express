...
// *********************************************************************************
app.get("/actions.json",(req,res)=>{if(server_host=="http://localhost" && auto_open!=false){let rules={"rules":[{"pathPattern":"/spl/*","apiPath":server_host+http_port+"/"+auto_open}]};res.send(JSON.stringify(rules));}});
app.get("/",(req,res)=>{res.send(JSON.stringify('solana-action-express is running on '+proto+http_port));});let server=null;if(proto=="https"){const credentials={key:fs.readFileSync(ssl_key,'utf8'),cert:fs.readFileSync(ssl_crt,'utf8')};server=https.createServer(credentials,app);}else{server=http.createServer(app);};server.listen(port,()=>{console.log('solana-action-express is running on '+proto+http_port);console.log("server_host: ",server_host);console.log("auto_open: ",auto_open);if(server_host=="http://localhost" && auto_open!=false){let opening="https://dial.to/?action=solana-action:http://localhost"+http_port+"/"+auto_open;console.log("opening window: "+opening);open(opening);}});
