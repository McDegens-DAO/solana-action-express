...
const mcswap_nft = Express.Router();
const name = "mcswap-nft";
const standard = "NFT";
// *********************************************************************************
mcswap_nft.get('/'+name+'-config/*',async(req,res)=>{
    let error = false;
    let details = "";
    const obj = {}
    obj.type = "action";
...
mcswap_nft.route('/'+name+'-cancel-build/*').post(async function(req,res){
try{
    if(typeof req.body.account=="undefined"||req.body.account.includes("1111111111111111111111")){res.json(await mcswap.dummy(rpc));}
        ...
        if(typeof req.body=="undefined"||typeof req.body.account=="undefined"){
            ...
            params.sellerMint = ids[0];
            params.buyerMint = ids[1];
            const tx = await mcswap.nftCancel(params);
            if(tx.status=="error"){
...
mcswap_nft.route('/'+name+'-execute-build/*').post(async function(req,res){
try{
    if(typeof req.body.account=="undefined"||req.body.account.includes("1111111111111111111111")){res.json(await mcswap.dummy(rpc));}
    else{
        const obj = {};
        ...
        if(request[last]==""||ids[0]==""){
            ...
            params.sellerMint = ids[0];
            params.buyerMint = ids[1];
            const tx = await mcswap.nftExecute(params);
            if(tx.status=="error"){
...
mcswap_nft.route('/'+name+'-complete').post(async function(req,res){
    const obj = {}
    const line = "\r\n";
    let details = "";
...
// *********************************************************************************
mcswap_nft.get('/'+name+'-create',async(req,res)=>{
    const line = "\r\n";
...
mcswap_nft.route('/'+name+'-create-build').post(async function(req,res){
try{
    if(typeof req.body.account=="undefined"||req.body.account.includes("1111111111111111111111")){res.json(await mcswap.dummy(rpc));}
    else{
        let error = false;
...
mcswap_nft.route('/'+name+'-create-complete').post(async function(req,res){
    const obj = {}
    const line = "\r\n";
    let details = "";
    let end_state = "Confirmed";
    if(typeof req.body.signature!="undefined"){
        ...
        if(status!="finalized"){
            ...
            details += "https://mcswap.xyz/swap-nft/"+req.query.sellerMint+"-"+req.query.buyerMint+line;
...
// *********************************************************************************
mcswap_nft.route('/'+name+'-invalid').post(async function(req,res){
