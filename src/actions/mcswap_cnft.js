...
const mcswap_cnft = Express.Router();
const name = "mcswap-cnft";
const standard = "CNFT";
// *********************************************************************************
mcswap_cnft.all('/'+name+'-config/*',async(req,res)=>{
    let error = false;
    let details = "";
    const obj = {}
    obj.type = "action";
...
mcswap_cnft.route('/'+name+'-cancel-build/*').post(async(req,res)=>{
try{
...
mcswap_cnft.route('/'+name+'-execute-build/*').post(async(req,res)=>{
try{
    if(typeof req.body.account=="undefined"||req.body.account.includes("1111111111111111111111")){res.json(await mcswap.dummy(rpc));}
    else{
        const obj = {};
...
mcswap_cnft.route('/'+name+'-complete').post(async(req,res)=>{
    const obj = {}
    const line = "\r\n";
    let details = "";
...
// *********************************************************************************
mcswap_cnft.all('/'+name+'-create',async(req,res)=>{
    const line = "\r\n";
...
mcswap_cnft.route('/'+name+'-create-build').post(async(req,res)=>{
try{
    if(typeof req.body.account=="undefined"||req.body.account.includes("1111111111111111111111")){res.json(await mcswap.dummy(rpc));}
    else{
        let error = false;
...
mcswap_cnft.route('/'+name+'-create-complete').post(async(req,res)=>{
    const obj = {}
    const line = "\r\n";
    let details = "";
    let end_state = "Confirmed";
...
// *********************************************************************************
mcswap_cnft.route('/'+name+'-invalid').post(async(req,res)=>{
